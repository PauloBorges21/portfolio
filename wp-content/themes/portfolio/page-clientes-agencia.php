<?php
//Template Name: Page Clientes Agência
...
<?php
        ...
        $getCliente = $_POST['getCliente'];
        $idPost = $_POST['idPost'];
        $imagemDestaque = $_POST['imagemDestaque'];
        $title = $_POST['title'];
        $contentDestaque = $_POST['content'];
...
$title = get_the_title($idPost, '', '', false);
 $content= get_the_content($idPost, '', '', false);
  $categories = get_the_category($idPost, '', '', false); 

?>


<div class="main-banner-interna">
  <img src="<?php echo $imagemDestaque?>" alt="" class="img-main-banner">
  ...
  <div class="content-main-banner">
    <div class="box-content-main-banner" onclick="sliderModalVejaMais(this)" data-ix="abrir-modal-video-full">
      ...
      class="btn-play-mainbanner image-2" data-sliderModalVeja="SliderVejaMais-<?php echo $idPost ?>">
...
<form action="/" data-form="SliderVejaMais-<?php echo $idPost ?>">

<?php if (have_rows('slider_modal_post', $idPost)): while (have_rows('slider_modal_post', $idPost)) : the_row(); ?>

    <?php if (have_rows('imagem_slide_modal', $idPost)): while (have_rows('imagem_slide_modal', $idPost)) : the_row(); ?>

        <?php $imagemSliderModal = get_sub_field('imagem', $idPost); ?>

        <input hidden="" name="imagemModalVejaMais" value="<?php echo $imagemSliderModal ?>">

    <?php endwhile; else : endif; ?>

    <?php if (have_rows('url_slide_modal', $idPost)): while (have_rows('url_slide_modal', $idPost)) : the_row(); ?>
        <?php $urlSliderModal = get_sub_field('url', $idPost); ?>
...
<form action="/" data-form="Slider-<?php echo $getCliente ?>-<?php echo $idPost ?>">
