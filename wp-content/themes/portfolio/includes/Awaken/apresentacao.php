<div id="apresentacao" class="section-sliders">
    <h2 class="h2-interna">Apresentação</h2>
    <div data-animation="slide" data-duration="500" data-infinite="1" class="sllider-interna w-slider">
        <div class="mask-sllider-interna w-slider-mask">
            ...
            <?php  if( have_rows('apresentaCards',110)){ //Se existir banner

                while ( have_rows('apresentaCards',110) ) { the_row(); //Percorre Todos os banners

                    $getCliente = get_sub_field('nome_do_cliente');
                    $getLogo = get_sub_field('logo',110);
                    $getCliente = preg_replace('/[ -]+/' , '-' , $getCliente);
                    $str = strtolower($getCliente);
                    $exibe = get_sub_field('apresentacao');
